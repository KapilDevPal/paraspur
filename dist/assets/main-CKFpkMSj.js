...
`;document.addEventListener(`DOMContentLoaded`,()=>{let i=document.querySelector(`#header-container`),a=document.querySelector(`#footer-container`);i&&(i.innerHTML=e+t),a&&(a.innerHTML=n);let o=document.querySelector(`#mobile-menu-btn`);o&&o.addEventListener(`click`,()=>{alert(`Mobile menu feature coming soon!`)}),document.querySelectorAll(`a[href^="#"]`).forEach(e=>{e.addEventListener(`click`,function(e){e.preventDefault(),document.querySelector(this.getAttribute(`href`)).scrollIntoView({behavior:`smooth`})})});let s=document.querySelector(`#rss-news-container, #news-feed`);s&&r(s)});async function r(e){try{let t=await(await fetch(`https://api.rss2json.com/v1/api.json?rss_url=https://timesofindia.indiatimes.com/rssfeeds/-2128819658.cms`)).json();t.status===`ok`&&(e.innerHTML=t.items.slice(0,5).map(e=>`
        <div class="flex items-start space-x-6 pb-6 border-b border-slate-100 last:border-0 group">
          <div class="flex-1">
            <span class="text-[10px] font-black uppercase text-primary-600 tracking-widest mb-2 block">${new Date(e.pubDate).toLocaleDateString()}</span>
            ...
            <p class="text-sm text-slate-500 mt-2 line-clamp-2">${e.description.replace(/<[^>]*>?/gm,``)}</p>
          </div>
        </div>
      `).join(``))}catch(t){console.error(`Error fetching news:`,t),e.innerHTML=`<p class="text-slate-400 text-sm">Latest news updates will appear here soon.</p>`}}